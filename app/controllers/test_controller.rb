...
class TestController < ApplicationController
	...
	def index
		request = 'https://api.mastercard.com/atms/v1/atm?Format=XML&Country=USA&PostalCode=63146&PageOffset=0&PageLength=10'
		data = session['data'] = MasterpassData.new("config.yml")
		service = Mastercard::Common::Connector.new(@data.consumer_key, generate_private_key)
		service.do_request(request, 'GET')
		Rails.logger.debug "HOLA"
		Rails.logger.debug service.to_yaml
		@rs = service
	end
	...
	def bf_method
	    ...
	    @data.callback_domain = @host_name =  "http://localhost:3000"
	end
